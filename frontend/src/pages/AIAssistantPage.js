...
import MarkdownRenderer from '../components/MarkdownRenderer'; // <-- CORRECTED IMPORT
...
const AIAssistantPage = () => {
  ...
  const handleSend = async (e) => {
    ...
    setModuleState((prev) => ({ messages: [...prev.messages, userMessage], input: '' }));
    setIsLoading(true);

    try {
      const aiText = await getAIResponse(prompt);
      const aiMessage = { sender: 'ai', text: aiText };
      setModuleState((prev) => ({ messages: [...prev.messages, aiMessage] }));
    } catch (error) {
      const errorMessage = { sender: 'ai', text: 'Sorry, I encountered an error. Please try again.' };
      setModuleState((prev) => ({ messages: [...prev.messages, errorMessage] }));
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div key={index} className={`flex items-start gap-4 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-xl px-4 py-3 rounded-lg ${msg.sender === 'user' ? 'bg-blue-500/20' : 'bg-panel'}`}>
              <div className="text-sm text-gray-300 whitespace-pre-wrap">
                {/* Use the MarkdownRenderer component here */}
      ...
      <div className="mt-6">
        <form onSubmit={handleSend} className="relative">
          <input
            type="text"
            value={input}
            onChange={(e) => setModuleState({ input: e.target.value })}
