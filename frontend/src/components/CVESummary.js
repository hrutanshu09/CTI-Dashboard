...
const CVESummary = () => {
    ...
    const handleSearch = async (e) => {
        ...
        if (!cveId) return;
        setIsLoading(true);
        setCveData(null); // Clear previous data
            ...
            <div className="flex-grow min-h-[200px]">
              ...
              {!isLoading && !cveData && (
                <div className="flex items-center justify-center h-full text-gray-500">
                  <p>Enter a CVE ID to get details.</p>
                </div>
              )}
                  ...
                  <div className="space-y-4 animate-fade-in">
                      ...
                      <div>
                          ...
                          <div className="text-sm text-gray-300 bg-background/50 p-4 rounded-md border border-border min-h-[120px]">
                              <p>{summaryText}{isTyping && <span className="inline-block w-2 h-4 bg-neon-blue animate-pulse ml-1 opacity-80"></span>}</p>
                          </div>
                      </div>
                  </div>
              )}
